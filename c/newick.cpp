...
NewickTree::NewickTree(const char *label_, double length_, std::vector<NewickTree *> *children_) {
    label = label_;
    length = length_;
    children = children_;
}
...
NewickTree *newick_from_json(Json &tree) {
    ...
    for (auto &child : tree["children"]) {
        children->push_back(newick_from_json(child));
    }
...
void NewickTree::dump_newick(FILE *outfile) {
    ...
    if (label.length() > 0) fprintf(outfile, "%s", label.c_str());
    if (length >= 0.0) fprintf(outfile, ":%lf", length);
